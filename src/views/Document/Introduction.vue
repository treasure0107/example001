<template>
  <div class="panel">
    <header class="panel-heading">平台介绍管理</header>
      ...
      <div>
        <el-table
          ...
          <el-table-column
            ...
            label="操作">
            <template slot-scope="scope">
              <el-button v-if="editShow" type="primary" size="mini" @click="goEdit('Int' ,scope.row.ID)">编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
//        POST /api/MarketType/UpdateByIDs
      getMarket(){
        try {
          let config = {
            method: "GET",
            url: baseUrl + "Document/QueryDocuments?pageindex=" + this.PageIndex + '&pagesize=' + this.PageSize + '&typeid=1',
          };
