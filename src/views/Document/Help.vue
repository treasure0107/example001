<template>
  <div class="panel">
    <header class="panel-heading">新手指导管理</header>
    <section style="padding-bottom: 30px;">
      <div style="padding: 20px;">
        <el-button type="success" @click="goEdit('Hel')" v-if="addShow">新增数据</el-button>
        ...
        <el-button type="danger" @click="delByIDs()" v-if="delShow">删除数据</el-button>
      </div>
      <div>
        <el-table
          ref="multipleTable"
          ...
          <el-table-column
            ...
            label="操作">
            <template slot-scope="scope">
              <el-button type="primary" size="mini" @click="goEdit('Hel', scope.row.ID)" v-if="editShow">编辑</el-button>
              <el-button type="danger" size="mini" @click="delByIDs(scope.row.ID)" v-if="delShow">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
//        POST /api/MarketType/UpdateByIDs
      ...
      updateByIDs(Status){
        ...
        try {
          let config = {
            method: "POST",
            url: baseUrl + "Document/UpdateDocumentStatusByIDs",
            data:{
              RecordIds: RecordIds,
              Status: Status
            }
      ...
      getMarket(){
        try {
          let config = {
            method: "GET",
            url: baseUrl + "Document/QueryDocuments?pageindex=" + this.PageIndex + '&pagesize=' + this.PageSize + '&typeid=2',
          };
...
<style lang="scss" scoped>
  ...
  .sp {display: block;  white-space:nowrap; overflow:hidden; text-overflow:ellipsis;}
</style>
